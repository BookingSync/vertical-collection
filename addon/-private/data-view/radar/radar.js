...
export default class Radar {
  constructor(
    ...
    this._occludedContentBefore.addEventListener('click', this.pageLeft.bind(this));
    this._occludedContentAfter.addEventListener('click', this.pageRight.bind(this));
  ...
  _updateVirtualComponents() {
    ...
    // Set padding element heights and widths.
    _occludedContentBefore.style.height = `${Math.max(renderedTotalBefore, 0)}px`;
    _occludedContentBefore.style.width = `${Math.max(renderedTotalBefore, 0)}px`;
    ...
    _occludedContentAfter.style.width = `${Math.max(renderedTotalAfter, 0)}px`;
  ...
  pageRight() {
    if (this.renderAll) {
      return; // All items rendered, no need to page right
    }
